...
build-container $image_name="" $fedora_version="" $variant="" $github="":
    ...
    KERNEL_VERSION="$(skopeo inspect docker://{{ IMAGE_REGISTRY_SRC }}/akmods@$AKMODS_DIGEST | jq -r '.Labels["ostree.linux"]')"
    LABELS=(
    ...
    pull-retry "{{ IMAGE_REGISTRY_SRC }}/akmods:main-$fedora_version@$AKMODS_DIGEST"
    pull-retry "{{ IMAGE_REGISTRY_SRC }}/akmods-nvidia-open:main-$fedora_version@$AKMODS_NVIDIA_DIGEST"
    ...
    # CI Cleanup
    if [[ -n "${CI:-}" ]]; then
        {{ PODMAN }} rmi -f "{{ IMAGE_REGISTRY_SRC }}/akmods:main-$fedora_version@$AKMODS_DIGEST"
        {{ PODMAN }} rmi -f "{{ IMAGE_REGISTRY_SRC }}/akmods-nvidia-open:main-$fedora_version@$AKMODS_NVIDIA_DIGEST"
...
gen-tags $image_name="" $fedora_version="" $variant="":
    ...
    while [[ ! -s "$LIST_TAGS" ]]; do
        skopeo list-tags docker://{{ IMAGE_REGISTRY_SRC }}/$image_name > "$LIST_TAGS"
    done
